...
int main(int argc, char** argv)
{
  ...
  const int Nt=100;
  const float dt=T/Nt;
  const float hp=2.*hc*(a1+a2)/(a1*a2);
  
  // Solution vector, initialized at 0 (environment temperature)
  ...
  std::vector<double> a(M+1,1.+dt*hp+2.*k*dt/(h*h)),b(M,-k*dt/(h*h)),c(M,-k*dt/(h*h));
  a[0]=1.;
  b[0]=0.;
  a[M]=1.;
  c[M-1]=-1.;
 ...
 // Analitic solution
    ...
    vector<double> thetaa(M+1);
     for(int m=0;m <= M;m++)
       thetaa[m]=Te+(To-Te)*cosh(sqrt(act)*(1-m*h/L))/cosh(sqrt(act));
 ...
 for(int i=0;i<Nt/10;i++){
 	...
 	for(int j=0;j<10;j++){
	 	// Solving the system a number of times
